...
local function set_language(self, lang)
	local localization=self.data.read(self.data.path..lang..'.json')
	if not localization then error('Non-existent language "'..lang..'"', 2) end
	self.data.language=lang
	self.data.text=M.decode(self.data.funlutab, self.data.json.decode(localization))
	return self.data.params[self.data.language]
end
...
---@param funlutab funlutab version 2.1.1 - latest 2.x
---@param json table json module https://github.com/rxi/json.lua
